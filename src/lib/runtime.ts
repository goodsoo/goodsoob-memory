// T3 — 런타임 seam: Tauri 탈출구(gh/claude/curl/zip/asset)를 실행 환경별로 분기한다.
//
// 폰·air·Pro 브라우저는 CLI(gh/claude/curl/zip)를 직접 못 돌린다 → 서버(server/index.ts,
// mac 위에서 구동)가 대신 실행하고 결과를 HTTP 로 돌려준다. Tauri 데스크탑은 T8 삭제
// 전까지 기존 경로(@tauri-apps/plugin-shell 의 Command)를 그대로 쓴다.
//
// selectAdapter 와 같은 런타임 감지(isTauri)를 재사용해 한 곳에서만 분기한다. 각
// 탈출구 모듈(gh.ts/claude.ts/imageDownload.ts/backup.ts/assetUrl.ts)은 여기를 거친다.
//
...
// ── 공통 결과 타입 (Tauri Command output 과 동형) ─────────────────────────────
...
// Tauri: Command.create(program, args).execute().
...
//
// Tauri: Command spawn + stdout/stderr 'data' 이벤트를 chunk 로 흘림.
// 서버:  POST /api/shell/stream (SSE) — event: chunk / event: done.
//
// onChunk 로 stdout/stderr 라인이 도착한다(개행 재부착은 호출부 책임 — 기존 claude.ts
// 동작 보존). cancel 은 Tauri child.kill() 또는 SSE abort → 서버가 프로세스 kill.
// ─────────────────────────────────────────────────────────────────────────────
export function runShellStream(
  ...
  if (isTauri) {
    return tauriShellStream(program, args, onChunk);
  }
  return serverShellStream(program, args, onChunk);
}

function tauriShellStream(
  program: string,
  args: string[],
  onChunk: (chunk: StreamChunk) => void,
): ShellStreamController {
  let stderrBuf = "";
  let canceled = false;
  // Child 타입은 dynamic import 라 unknown — kill 만 부른다.
  ...
  const done = new Promise<ShellResult>((resolve) => {
    ...
    const finish = (code: number | null) => {
      if (settled) return;
      settled = true;
      resolve({ stdout: "", stderr: stderrBuf.trim(), code });
    };

    void import("@tauri-apps/plugin-shell").then(({ Command }) => {
      ...
      cmd.stderr.on("data", (chunk: string) => {
        stderrBuf += chunk + "\n";
        onChunk({ stream: "stderr", data: chunk });
      });
      ...
      cmd
        .spawn()
        .then((child) => {
          childRef = child as unknown as { kill: () => Promise<void> };
          if (canceled) void childRef.kill().catch(() => {});
        })
        .catch((err) => {
          stderrBuf += String(err) + "\n";
          finish(null);
...
//
// Tauri: convertFileSrc(abs) → asset:// URL (fs scope 안).
// 서버:  /api/attachment?path=<relPath> (서버가 VAULT_DIR 밑 파일을 서빙, traversal 가드).
//
// 입력은 vaultRoot(절대) + relPath(vault 상대). Tauri 는 abs 가 필요, 서버는 rel 만.
...
export function attachmentUrl(
  ...
  if (isTauri) {
    // dynamic import 안 됨(동기 함수) — 정적 import 는 Tauri 없는 환경에서도 로드되지만
    // convertFileSrc 자체는 순수 함수라 호출만 Tauri 에서. 브라우저면 아래 서버 경로.
    return tauriAssetUrl(vaultRoot, relPath);
  }
  // 브라우저 — 서버 attachment 엔드포인트. relPath 만 필요(서버가 VAULT_DIR 붙임).
...
// convertFileSrc 를 게으르게 참조 — 정적 import 하면 브라우저 번들에도 들어가지만
// 실행은 isTauri 에서만. 모듈 로드 시점 부작용이 없어 안전.
import { convertFileSrc } from "@tauri-apps/api/core";
function tauriAssetUrl(vaultRoot: string | null, relPath: string): string {
