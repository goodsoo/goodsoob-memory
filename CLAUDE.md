...
- 🟡 V0.8 — **PWA local-first 전환** (브랜치 `feat/pwa-local-first`, design: `docs/designs/pwa-local-first-migration.md`). Tauri 데스크탑 셸 폐기 → **브라우저/PWA + 로컬 Bun 서버 단일 아키텍처**. **완료**: T0 git gc(.git churn 해소) / T1~T4 Bun 서버·HTTP adapter·PWA 셸·launchd 상주(localhost:7080) / **T8 Tauri 전면 삭제** — `src-tauri/` 통삭 + `@tauri-apps` 의존·스크립트 제거 + isTauri 분기 서버 경로로 통일 + vault-add Dialog 제거(단일 vault) + export→브라우저 download + `--header-h` web 높이(3.5rem) 단일화 / **gcal 통삭** (Google Calendar 는 tower 로 이전, 이 앱에서 제거 — 로컬 일정 schedules 는 유지) / portfolio **scanAll** batch (카드 스캔 985왕복 → 1회). **미완**: T5 capture outbox(IndexedDB) / T6 읽기 캐시 / T7 데이터 유실 스위트 / auto-backup·portfolio auto-sync 의 서버 경유 활성화 결정(현재 `isTauri` 게이트로 보존·비활성, App.tsx:250·265) / 폰 PWA tailscale 노출.
